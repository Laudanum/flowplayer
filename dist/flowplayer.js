/*!
   ...
   Flowplayer v5.3.2 (2013-02-21) | flowplayer.org/license
...
}(jQuery);var MEDIATYPES = "audio,video";
...
$.fn.flowplayer = function(opts, callback) {
   ...
   return !opts && this.data("flowplayer") || this.each(function() {
      ...
      var root = $(this).addClass("is-loading"),
         ...
         videoTag = $(MEDIATYPES, root).addClass("fp-engine").removeAttr("controls"),
...
!function() {
   ...
   var s = flowplayer.support,
      browser = $.browser,
      audio = $("<audio loop autoplay preload/>")[0],
...
function getType(type, tag) {
   if (!tag) tag = "video";
   return /mpegurl/i.test(type) ? "application/x-mpegurl" : tag + "/" + type;
}

function canPlay(type) {
   // check audio and video
   var mediatypes = MEDIATYPES.split(",");
   var mediaobjects = [AUDIO, VIDEO];
   var result = null;
   for (var i = 0; i < mediatypes.length; i++) {
      var tag = mediatypes[i];
      var re = new RegExp("^(" + tag + "|application)","g");
      if (!re.test(type))
         type = getType(type, tag);
      result = !!mediaobjects[i].canPlayType(type).replace("no", '');
      // if this is true return early
      if (result) return result;
   }
   // otherwise just return whatever result is
   return result;
}
   ...
   function listen(api, sources, video) {
      ...
      $.each(EVENTS, function(type, flow) {
         ...
         api.addEventListener(type, function(e) {
            ...
            if (!player.ready && !/ready|error/.test(flow) || !flow || !$(MEDIATYPES, root).length) { return; }
   ...
   if (player.conf.generate_cuepoints) {
      ...
      player.bind("ready", function() {
         ...
         $.each(cues, function(i, cue) {
            ...
            if (player.conf.cuepoints_are_percentages) {
               time = cue.time * duration || cue * duration;
               if (time < 0) time = duration + cue * duration;
               cues[i].time = time;
            }
