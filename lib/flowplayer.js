...
// auto-install (any audio or video tag with parent .flowplayer)
$(function() {
   if (typeof $.fn.flowplayer == 'function') {
      $(MEDIATYPES).parent(".flowplayer").flowplayer();
   }
});
   ...
   return !opts && this.data("flowplayer") || this.each(function() {
      ...
      if (!root.data('flowplayer')) { // Only bind once
         root.bind("boot", function() {
            ...
            var videoTagName = videoTag[0].tagName.toLowerCase();
            var is_audio = videoTagName == 'audio';

            // splash
            if (conf.splash || root.hasClass("is-splash") || !flowplayer.support.firstframe) {
               // not for audio as this hides the control bar which is the only visible element for audio
               if ( ! is_audio ) {
                  api.forcedSplash = !conf.splash && !root.hasClass("is-splash");
                  api.splash = conf.splash = conf.autoplay = true;
                  root.addClass("is-splash");
               } else {
                  // api.forcedSplash = true;//!conf.splash && !root.hasClass("is-splash");
                  root.addClass("is-audio");
                  //root.addClass("is-splash");
                  // without splash we can't see the click ui
                  conf.splash = conf.autoplay = true;
                  // conf.autoplay = false;
                  // set the ratio
                  // if (! conf.ratio ) conf.ratio = 0;
               }
